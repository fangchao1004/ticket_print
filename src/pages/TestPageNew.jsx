...
export default function TestPageNew() {
    ...
    const getRenderViewByList = useCallback(() => {
        ...
        if (ticketValue.pages) {
            ...
            return ticketValue.pages.map((_, index) => {
                return <div style={ticketValue.scal ? {} : { width: 840 * scale_value, height: 1188 * scale_value }}>
                    <div style={{ transform: `scale(${scale_value})` }}>
                        <RenderEngine
                            jsonlist={ticketValue}
                            page={index}
                            scaleNum={scalObj.scaleNum || 1}
                            bgscalNum={scalObj.bgscalNum || 1}
                        />
                    </div>
                </div>
            })
    ...
    return (
        <div style={{ ...styles.root, marginTop: ticketValue.scal ? 0 : -40 }}>
            {getRenderViewByList()}
        </div>
    )
}
